...
import time
from config import MODEL_OPTIONS, STYLE_OPTIONS, ASPECT_RATIOS
from utils import translate_to_english, save_to_library, load_library
from ai_engine import call_ollama, generate_image_stability
    ...
    with tab1:
        ...
        if generate_btn and user_input:
            ...
            with st.spinner("🤖 AI promptunuzu optimize ediyor..."):
                ...
                if "error" in result:
                    st.error(f"⚠️ **Bağlantı Sorunu:** {result['error']}")
                    st.info("💡 **Çözüm:** Bilgisayarınızda Ollama uygulamasını başlatın ve modelin yüklü olduğundan emin olun.")
                else:
                    # Sonuçları kaydet
